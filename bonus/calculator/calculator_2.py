...
class BonusTimeCalculator:
    ...
    def _check_date(self, start_date, end_date) -> None:
    ...
    def _retrieve_bonus_objects(self) -> list:
    ...
    def _calculate_bonus(
        ...
        for bonus in bonus_list:
            if bonus.is_percentage:
                bonus = self._calculate_bonus_percentage(bonus, now)
                bonus_amount += bonus
            else:
                bonus = self._calculate_bonus_amount(bonus, now)
                bonus_amount += bonus

    def _calculate_bonus_percentage(
            self,
            bonus: Bonus,
            now: datetime.datetime
    ) -> Decimal:
        salary = self._get_salary(bonus)
        if bonus.end_date >= now:
            partial_bonus = self._do_partial_calculation(
                bonus,
                now,
                is_percentage=True
            )
            bonus = partial_bonus * salary
        elif bonus.start_date <= self._start_date and bonus.end_date <= now:
            bonus = bonus.value * salary
        return bonus
